...
Func algorithm_AllTroops() ;Attack Algorithm for all existing troops
	...
	LaunchTroop2($listInfoDeploy, $g_iClanCastleSlot, $g_iKingSlot, $g_iQueenSlot, $g_iWardenSlot, $g_iChampionSlot, $g_iMinionPSlot, $g_iDragonDukeSlot)
	...
	For $x = 0 To 1
		...
		EndIf
		For $i = $eBarb To $eRamR ; launch all remaining troops
...
Func SetSlotSpecialTroops()
	...
	For $i = 0 To UBound($g_avAttackTroops) - 1
		...
		ElseIf $g_avAttackTroops[$i][0] = $eMinionP Then
			$g_iMinionPSlot = $i
		ElseIf $g_avAttackTroops[$i][0] = $eDragonDuke Then
			$g_iDragonDukeSlot = $i
		EndIf
	Next
	...
	If $g_bDebugSetlog Then
		...
		SetDebugLog("SetSlotSpecialTroops() Dragon Duke Slot: " & $g_iDragonDukeSlot, $COLOR_DEBUG)
